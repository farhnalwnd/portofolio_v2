<template>
  <nav class="sticky top-0 z-50 bg-brutal-cream border-b-3 border-brutal-black px-4 md:px-8 py-4 flex items-center justify-between">
    ...
    <div class="flex items-center gap-4">
      <NuxtLink to="/catch-me" class="hidden sm:inline-block">
        <BrutalistBtn size="sm" color="yellow">Hire Me</BrutalistBtn>
      </NuxtLink>

      <!-- Mobile Menu Button -->
      <button 
        class="md:hidden flex items-center justify-center p-2 border-3 border-brutal-black bg-white hover:bg-brutal-yellow transition-colors shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
        @click="toggleMenu"
      >
    ...
    <div 
      v-if="isOpen"
      class="md:hidden fixed top-[67px] left-0 w-full h-[calc(100vh-67px)] bg-brutal-black/50 backdrop-blur-sm z-40"
      @click="closeMenu"
    >
      ...
      >
        ...
        <div class="pt-6 border-t-3 border-brutal-black">
          <NuxtLink to="/catch-me" @click="closeMenu">
            <BrutalistBtn size="md" color="yellow" class="w-full text-center">Hire Me</BrutalistBtn>
          </NuxtLink>
        </div>
      </div>
    </div>
